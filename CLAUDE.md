...
- **`cloche`** (CLI) — Short-lived client. Talks to daemon over gRPC. Subcommands: `run`, `resume`, `status`, `list`, `logs`, `stop`, `delete`, `poll`, `loop`, `shutdown`, `console`, `project`, `init`, `health`, `get`, `set`, `tasks`, `activity`, `workflow`, `validate`, `doctor`, `debug`.
