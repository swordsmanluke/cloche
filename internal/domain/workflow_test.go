...
func TestWorkflow_Validate_DuplicateOutputMapping_DifferentKeys(t *testing.T) {
	wf := &domain.Workflow{
		Name: "test",
		Steps: map[string]*domain.Step{
			"src": {Name: "src", Type: domain.StepTypeScript, Results: []string{"ok"}},
			"a":   {Name: "a", Type: domain.StepTypeScript, Results: []string{"ok"}},
			"b":   {Name: "b", Type: domain.StepTypeScript, Results: []string{"ok"}},
			"c":   {Name: "c", Type: domain.StepTypeAgent, Results: []string{"done"}},
		},
		Wiring: []domain.Wire{
			{From: "src", Result: "ok", To: "a"},
			{From: "src", Result: "ok", To: "b"},
			{From: "a", Result: "ok", To: "c", OutputMap: []domain.OutputMapping{
				{EnvVar: "FOO", Path: domain.OutputPath{}},
			}},
			{From: "b", Result: "ok", To: "c", OutputMap: []domain.OutputMapping{
				{EnvVar: "BAR", Path: domain.OutputPath{}},
			}},
			{From: "c", Result: "done", To: domain.StepDone},
		},
		EntryStep: "src",
		Collects: []domain.Collect{
			{
				Mode:       domain.CollectAll,
				Conditions: []domain.WireCondition{{Step: "a", Result: "ok"}, {Step: "b", Result: "ok"}},
				To:         "c",
			},
		},
	}
	err := wf.Validate()
	assert.NoError(t, err, "different env var keys on wires to same step should be valid")
}

func TestWorkflow_Validate_DuplicateOutputMapping_SameKey(t *testing.T) {
	wf := &domain.Workflow{
		Name: "test",
		Steps: map[string]*domain.Step{
			"src": {Name: "src", Type: domain.StepTypeScript, Results: []string{"ok"}},
	...
	err := wf.Validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "duplicate output mapping")
	assert.Contains(t, err.Error(), "FOO")
	assert.Contains(t, err.Error(), "c")
}

func TestWorkflow_Validate_DuplicateOutputMapping_SameKeyDifferentSteps(t *testing.T) {
	wf := &domain.Workflow{
		Name: "test",
		Steps: map[string]*domain.Step{
			"a": {Name: "a", Type: domain.StepTypeScript, Results: []string{"ok"}},
			"b": {Name: "b", Type: domain.StepTypeAgent, Results: []string{"done"}},
			"c": {Name: "c", Type: domain.StepTypeAgent, Results: []string{"done"}},
		},
		Wiring: []domain.Wire{
			{From: "a", Result: "ok", To: "b", OutputMap: []domain.OutputMapping{
				{EnvVar: "FOO", Path: domain.OutputPath{}},
			}},
			{From: "a", Result: "ok", To: "c", OutputMap: []domain.OutputMapping{
				{EnvVar: "FOO", Path: domain.OutputPath{}},
			}},
			{From: "b", Result: "done", To: domain.StepDone},
			{From: "c", Result: "done", To: domain.StepDone},
		},
		EntryStep: "a",
	}
	err := wf.Validate()
	assert.NoError(t, err, "same env var key on wires to different steps should be valid")
}

func TestWorkflow_Validate_DuplicateOutputMapping_NoMappings(t *testing.T) {
	wf := &domain.Workflow{
		Name: "test",
		Steps: map[string]*domain.Step{
			"a": {Name: "a", Type: domain.StepTypeScript, Results: []string{"ok"}},
