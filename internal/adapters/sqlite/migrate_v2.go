...
func migrateRunsCompositeKey(db *sql.DB) error {
	...
	stmts := []string{
		`CREATE TABLE IF NOT EXISTS runs_new (
			...
			parked_thread_id TEXT NOT NULL DEFAULT '',
			parked_title TEXT NOT NULL DEFAULT '',
		...
		`INSERT OR IGNORE INTO runs_new
			(id, workflow_name, state, active_steps, started_at, completed_at,
			 ...
			 parent_step_name, parked_thread_id, parked_title, is_builtin, user_initiated, repository, repositories)
		 SELECT id, workflow_name, state, active_steps, started_at, completed_at,
			...
			COALESCE(is_builtin,0), COALESCE(user_initiated,0), COALESCE(repository,''), COALESCE(repositories,'')
		 FROM runs`,
		`DROP TABLE runs`,
