...
func migrate(db *sql.DB) error {
	...
	alterStmts := []string{
		...
		`ALTER TABLE runs ADD COLUMN parked_thread_id TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE runs ADD COLUMN parked_title TEXT NOT NULL DEFAULT ''`,
...
// CountDoneRunsByProjectGroupedByRepo implements ports.DoneRepoCounter: a
...
func (s *Store) UpsertPoll(ctx context.Context, record *ports.PollRecord) error {
	_, err := s.write.ExecContext(ctx,
		...
		record.RunID, record.StepName,
		formatTime(record.StartedAt), formatTime(record.LastPollAt),
		record.PollCount,
	)
	return err
}
...
func (s *Store) GetPoll(ctx context.Context, runID, stepName string) (*ports.PollRecord, error) {
	...
	r.LastPollAt = parseTime(lastPollAt)
	return r, nil
}
