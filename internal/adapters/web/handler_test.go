...
`
	...
	// Should have 13 wires: 7 explicit + 3 implicit timeout->abort + 3 implicit token-limit->abort (one per step)
	assert.Len(t, pipeline.Wires, 13)
	...
	// analyze:error->abort, transform:fail->abort, validate:fail->abort = 3 explicit
	// + 3 implicit timeout->abort + 3 implicit token-limit->abort = 9
