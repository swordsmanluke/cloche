...
func TestServer_Orchestrate(t *testing.T) {
	...
	// Create host.cloche with a simple script step
	hostWorkflow := `workflow "main" {
  step hello {
    run     = "echo hello"
...
`
	require.NoError(t, os.MkdirAll(filepath.Join(dir, ".cloche"), 0755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".cloche", "host.cloche"), []byte(hostWorkflow), 0644))

	srv := server.NewClocheServer(store, nil)

	resp, err := srv.Orchestrate(context.Background(), &pb.OrchestrateRequest{ProjectDir: dir})
	require.NoError(t, err)
	assert.NotEmpty(t, resp.RunId, "should return a run ID")

	// Poll until the host workflow completes
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		run, err := store.GetRun(context.Background(), resp.RunId)
	...
	assert.True(t, run.IsHost)
}

func TestServer_Orchestrate_MissingHostCloche(t *testing.T) {
	store, err := sqlite.NewStore(":memory:")
	require.NoError(t, err)
	defer store.Close()
	...
	_, err = srv.Orchestrate(context.Background(), &pb.OrchestrateRequest{ProjectDir: dir})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "host.cloche not found")
}

func TestServer_Orchestrate_EmptyProjectDir(t *testing.T) {
	store, err := sqlite.NewStore(":memory:")
	require.NoError(t, err)
	defer store.Close()
	...
	_, err = srv.Orchestrate(context.Background(), &pb.OrchestrateRequest{ProjectDir: ""})
	assert.Error(t, err)
