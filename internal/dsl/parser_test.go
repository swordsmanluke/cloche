...
func TestParser_WorkflowRepos(t *testing.T) {
	input := `workflow "multi-repo" {
  ...
  build:fail    -> abort
	...
	wf, err := dsl.Parse(input)
	require.NoError(t, err)
	require.Equal(t, []string{"backend", "frontend"}, wf.Repos)
}

func TestParser_WorkflowRepos_Empty(t *testing.T) {
	input := `workflow "simple" {
  step run {
    run     = "echo hi"
    results = [success]
  ...
  run:success -> done
	...
	wf, err := dsl.Parse(input)
	require.NoError(t, err)
	require.Empty(t, wf.Repos)
}

func TestParser_StepRepositoryField(t *testing.T) {
	input := `workflow "deploy" {
  repos = ["backend"]
  step build {
    run        = "make build"
    repository = "backend"
  ...
  build:fail    -> abort
	...
	step := wf.Steps["build"]
	require.NotNil(t, step)
	assert.Equal(t, "backend", step.Config["repository"])
}

func TestParser_ParseRepositoriesFrom(t *testing.T) {
	input := `repository "backend" { path = "../backend", url = "https://github.com/org/backend", default = true }
repository "frontend" { path = "../frontend" }
workflow "develop" {
  step run {
    run     = "echo hi"
    results = [success]
  ...
  run:success -> done
	...
	repos, err := dsl.ParseRepositoriesFrom(input)
	require.NoError(t, err)
	require.Len(t, repos, 2)
	assert.Equal(t, "backend", repos[0].Name)
	assert.Equal(t, "../backend", repos[0].Path)
	assert.Equal(t, "https://github.com/org/backend", repos[0].URL)
	assert.True(t, repos[0].Default)
	assert.Equal(t, "frontend", repos[1].Name)
	assert.Equal(t, "../frontend", repos[1].Path)
	assert.False(t, repos[1].Default)
}

func TestParser_ParseRepositoriesFrom_Empty(t *testing.T) {
	input := `workflow "develop" {
  step run {
    run     = "echo hi"
    results = [success]
