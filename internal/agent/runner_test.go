...
func TestRunner_ConfigAgentCommands(t *testing.T) {
	dir := t.TempDir()

	// Create a mock agent script
	mockAgent := filepath.Join(dir, "mock-agent.sh")
	require.NoError(t, os.WriteFile(mockAgent, []byte("#!/bin/sh\ncat > /dev/null\necho 'config agent ran'\n"), 0755))

	// Write config with agent_commands pointing to mock agent
	clocheDir := filepath.Join(dir, ".cloche")
	require.NoError(t, os.MkdirAll(clocheDir, 0755))
	require.NoError(t, os.WriteFile(filepath.Join(clocheDir, "config"), []byte(`
[daemon]
agent_commands = "`+mockAgent+`"
`), 0644))

	// Workflow with NO agent_command set — should use config
	workflowContent := `workflow "config-test" {
  step implement {
    prompt = "Write some code."
    results = [success, fail]
  }
  ...
  implement:fail -> abort
	...
	workflowPath := filepath.Join(dir, "config-test.cloche")
	require.NoError(t, os.WriteFile(workflowPath, []byte(workflowContent), 0644))
...
func TestRunner_PriorityChain_WorkflowOverridesConfig(t *testing.T) {
	dir := t.TempDir()

	// Config agent (should NOT be used)
	configAgent := filepath.Join(dir, "config-agent.sh")
	require.NoError(t, os.WriteFile(configAgent, []byte("#!/bin/sh\ncat > /dev/null\necho 'CLOCHE_RESULT:fail'\n"), 0755))

	// Workflow agent (should be used)
	workflowAgent := filepath.Join(dir, "workflow-agent.sh")
	require.NoError(t, os.WriteFile(workflowAgent, []byte("#!/bin/sh\ncat > /dev/null\necho 'CLOCHE_RESULT:success'\n"), 0755))

	// Write config with config agent
	clocheDir := filepath.Join(dir, ".cloche")
	require.NoError(t, os.MkdirAll(clocheDir, 0755))
	require.NoError(t, os.WriteFile(filepath.Join(clocheDir, "config"), []byte(`
[daemon]
agent_commands = "`+configAgent+`"
`), 0644))

	// Workflow overrides with workflow agent
	workflowContent := `workflow "priority-test" {
  container {
    agent_command = "` + workflowAgent + `"
  }
  ...
  implement:fail -> abort
	...
	workflowPath := filepath.Join(dir, "priority.cloche")
	require.NoError(t, os.WriteFile(workflowPath, []byte(workflowContent), 0644))
	...
	// Workflow agent returns success; config agent would return fail
	...
	assert.Equal(t, "success", stepResult, "workflow-level agent_command should override config")
}

func TestRunner_PriorityChain_EnvOverridesConfig(t *testing.T) {
	dir := t.TempDir()
	...
	// Env agent (should be used)
	envAgent := filepath.Join(dir, "env-agent.sh")
	require.NoError(t, os.WriteFile(envAgent, []byte("#!/bin/sh\ncat > /dev/null\necho 'CLOCHE_RESULT:success'\n"), 0755))
...
`), 0644))
	...
	// Set env var (higher priority than config)
	t.Setenv("CLOCHE_AGENT_COMMAND", envAgent)

	workflowContent := `workflow "env-test" {
  step implement {
    prompt = "Write some code."
    results = [success, fail]
  }
  ...
  implement:fail -> abort
	...
	workflowPath := filepath.Join(dir, "env-test.cloche")
	require.NoError(t, os.WriteFile(workflowPath, []byte(workflowContent), 0644))
	...
	assert.Equal(t, "success", stepResult, "env var should override config")
}
