...
The `host.cloche` file is the orchestration layer that ties everything together.
When you run `cloche init`, a minimal `host.cloche` with just a `main` workflow is
generated. As your project grows, you can add `list-tasks` and `finalize` workflows
to automate task discovery and cleanup.

The daemon recognizes three workflow names in `host.cloche` and runs them in order:
...
The `main` workflow does the actual work. This is the only required workflow in
`host.cloche` and is always generated by `cloche init`.
