...
`repos` is a list of repository names declared in `config.toml`. It is visible in
`cloche project`, and the runtime enforces it: only the declared repositories are
copied into the workflow's container and extracted into result branches. Workflows
sharing a `container.id` share one container copy containing the union of their
`repos` declarations (if any of them declares no `repos`, all repositories are
included). Omitting `repos` includes every configured repository.
...
Log streaming is backed by `internal/logstream`. Inside the container, a `Writer` records timestamped, type-prefixed entries (`status`, `script`, `llm`) to `full.log`. On the daemon side, a `Broadcaster` fans log lines to multiple concurrent subscribers (CLI follow mode, web dashboard live view), retaining an in-memory history for each active run; the history is released when the run finishes. Each run's retained history is capped at `logstream.DefaultHistoryCap` (5000 lines) — older lines are evicted from memory as new ones arrive, but stay readable from the on-disk `full.log`. Each line gets a sequence number, so a dashboard subscriber that reconnects (e.g. after a network blip) resumes from where it left off instead of replaying everything; if the gap has fallen out of the retained window, the subscriber is told to resync rather than being served a silently incomplete stream. The broadcaster runs for the lifetime of the workflow run and is closed when the run completes or the daemon shuts down. Each log line is parsed for tool-call blocks (`ParseClaudeStream`) before being forwarded to subscribers so the web dashboard can format agent output distinctly from plain script output.
...
Stop all active runs for a task. Container runs have their container terminated; host runs (including user-initiated runs) have their execution cancelled gracefully — if the active step declares a `fail` result, the engine walks the fail-branch wires (e.g. cleanup or unclaim steps) before the run transitions to `cancelled`.
