...
`update-docs.md`), a `version` file (starting at `1`), an orchestration
workflow (`host.cloche`), and a prompt generation script
(`scripts/prepare-prompt.sh`). Skips files that already exist.
...
│   ├── version               # Project version integer (incremented by evolution)
│   ├── host.cloche           # Orchestration workflow (runs on host)
│   ├── scripts/
│   │   └── prepare-prompt.sh # Default prompt generator for orchestration
│   ├── prompts/
