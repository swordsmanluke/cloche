...
- `45b1238` (+ many follow-ups) Adds the **vertical development workflow** for layered feature delivery: `cloche run --workflow vertical` walks a feature task through BDD test-plan, layered implementation (each layer becomes its own PR), docs, and finalize phases. Each phase opens a PR the user must approve before the next phase begins. See `docs/design/vertical-workflow.md`.
