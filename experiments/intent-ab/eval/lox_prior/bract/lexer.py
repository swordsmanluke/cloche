"""Bract lexer -- deliberately Lox-prior calibration variant (E3).

Differs from the spec (DESIGN.md sec2, Q4) in exactly one place: it accepts
`!=` as a valid not-equals spelling (a Lox/C-family habit) instead of
lex-erroring on the leading `!`. Everything else is a faithful lex per the
grammar, since the point of this calibration implementation is to be wrong
*only* on the prior-trap dimensions, not to fail unrelated syntax.
"""
...
def lex(source: str) -> list:
    ...
    while i < n:
        ...
        if two == "<>":
            ...
            continue
        if two == "!=":
            # Lox prior: accept != as a not-equals spelling (Q4 trap).
            tokens.append(Token("NOTEQ", "!=", line))
            i += 2
            continue
        if two == "<=":
